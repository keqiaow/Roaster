...
$cuda_prefix = "${Env:ProgramFiles}/NVIDIA GPU Computing Toolkit/CUDA/v11.1"

Get-ChildItem ../nuget | Foreach-Object {
    $pkg = $_.Name
    if ($pkg -eq "cuda" -or $pkg -eq "cublas" -or $pkg -eq "cufft" -or $pkg -eq "cusparse")
    {
        $prefix = ${cuda_prefix}
    }
    elseif ($pkg -eq "cusolver" -or $pkg -eq "cusolverMg")
    {
        $prefix = ${cuda_prefix}
    }
